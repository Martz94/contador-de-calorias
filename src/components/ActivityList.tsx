...
export default function ActivityList({activities, dispatch} : ActivityListProps){
  ...
  return (
    <>
      <h2 className=" text-4xl font-boldv text-slate-600 text-center uppercase font-bold">
        Comida y Actividades</h2>

      {isEmptyActivities ? <p className=" text-center my-5">No hay actividades aun...</p> :

        activities.map(activity => (
          <div key={activity.id} className=" px-5 py-10 bg-white rounded-xl mt-5 flex justify-between shadow">
            <div className=" space-y-2 relative">
              <p className={`absolute -top-8 px-10 py-2 text-white uppercase font-bold
